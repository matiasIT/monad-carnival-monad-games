...
export default function App() {
  ...
  const onCheckIn = async () => {
    ...
    try {
      const hash = await walletClient.sendTransaction({
        account: address as Address, to: address as Address, value: 0n
      })
  ...
  const fetchLeaderboard = async (): Promise<ScoreEntry[]> => {
    ...
    const raw = await publicClient.readContract({
      abi: ABI, address: CONTRACT_ADDRESS as Address,
      functionName: 'getTopScores', args: [100n]
    }) as readonly { player: Address; points: bigint; timestamp: bigint }[]
    const entries = raw.map(e => ({ player: e.player as Address, points: Number(e.points), timestamp: Number(e.timestamp) }))
